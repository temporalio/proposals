# Rust SDK Design Proposal

Applicable SDKs: Rust
Proposer: Spencer Judge
User Facing?: Yes

<aside>
⚠️ Please comment after reading the whole proposal. The ❓is used in callouts as a request for comment.
</aside>

We’ll follow the pattern of the last few SDK proposals where we have **phase 1** define the client, activity, worker, data converter APIs, etc and **phase 2** define the workflow APIs in further detail. This proposal is phase 1.

# Requirements & Goals

## Requirements

- Rust ≥ 1.89 - May increase during development. Generally speaking requiring a high MSRV isn’t an issue in the community.
- Avoid introducing new dependencies not already in Core. Core already pulls in a lot, so it’s unlikely much extra is needed here. We can aim to feature-gate extra functionality
- Workflows should be able to compile to WASM. Even though we aren’t likely to support them initially, care needs to be taken to not make this a nightmare when we do enable this, and that means structuring crates and/or feature flags to account for that early on.

## Goals

- Type Safety
    - The Rust SDK should be the most type-safe SDK yet. We have tons of tools available to ensure workflow/activity/nexus calls are compile-time guaranteed to accept & return the right things, and all types are serializable.
- Ergonomics
    - Rust is, generally, quite an ergonomic language… except when it’s not. Strive to make the APIs easy-to-use and natural.
    - We know we’ll be leaning on procmacros. Ensure that errors generated within them are clean and point to the right locations in source.
    - High quality rustdocs with doc examples. Rust’s doc features are very strong, and the community bar is *much* higher than in other languages, so we need to meet it.
- Speed - there’s no reason the Rust SDK can’t be, hands down, the most efficient SDK from a memory use & overhead perspective.

# Crates

I propose moving to the following new structure, keeping everything within the Core repo. I don’t want to break out the SDK yet into its own repo while iterating on it, but it probably makes sense to do before GA or possibly public preview. The main wrinkle here is that the SDK remains a very convenient way to test new Core features - that might need to be done by using branches in the future.

The `temporalio` prefix is used for consistency with our other SDKs

```mermaid
flowchart TD
    sdk[temporalio-sdk] --> core
    sdk --> client
    sdk --> common
    core[temporalio-core] --> common[temporalio-common]
    core --> temporalio-core-macros
    client[temporalio-client] --> common
    cbridge[temporalio-core-c-bridge] --> core
    cbridge --> common
    cbridge --> client
```

- `temporalio-common` - This will combine what is today `temporal-sdk-core-api` and `temporal-sdk-core-protos`, including options for telemetry/metrics configuration. Additionally, things like workflow and activity definition traits will need to live here since the client & sdk both need access to them.
  - Actual workflow code will need access to activity and other workflow definitions (for invoking them as activities/child workflows), thus this crate must be compilable in a WASM mode to support WASM workflows (which likely excludes protos, etc).
  - The rest of workflow utilities, like the contexts, etc, may also go in here simply to avoid massive feature-flag confusion in the SDK crate, where huge portions would need to be excluded for WASM compile mode.
- `temporalio-core-macros` is `rustfsm` renamed, and critically where all the workflow/activity/nexus procmacros will go. Procmacros must be defined in their own crate. Better to have one than a bunch. Right now `rustfsm` is actually two crates - the macro and a trait. The trait can be moved into core. Users won't need to depend on this crate directly, as the SDK/common crates will re-export its functionality.
- Everything else is just a ready-to-publish name of what exists today.
- The c bridge does not need to be published until someone asks.
- Modules within crates will be used more heavily than they are today to better organize things. I don’t go into detail on that structure here since it’ll require quite a bit of legwork to determine the best layout, but it’ll attempt to mirror other SDKs where appropriate.

<aside>
❓ Should we publish an umbrella `temporalio` crate? With `sdk`, `client` and `common` modules that re-export their respective crates? This definitely happens in the ecosystem (ex: the very popular [bevy](https://github.com/bevyengine/bevy/blob/main/crates/bevy_internal/src/lib.rs)). My take is probably not. `temporalio-sdk` itself can just have re-exports for client and common and that’s the 90% case.
</aside>
...
I have picked up [https://bon-rs.com/](https://bon-rs.com/) which will replace `derive_builder` everywhere it’s used currently. It’s a huge ergonomics improvement to builders and is extremely flexible.

## Feature flagging

Feature flags will be used liberally to eliminate optional dependencies where possible
...
For activities, the question is more up in the air. We’re definitely going to keep using tokio in the SDK itself - avoiding it would be a monumental amount of churn. So a tokio context *is* available to those who want to use it (which is likely many many people) in activities.

<aside>
❓ The question is should we use things like `CancellationToken` in our API? I think yes, simply to
avoid version clashes with people's own use of Tokio, and to avoid the problems we've had in other
SDKs where some change in the dependency causes us determinism issues. We've been bitten enough
times by that that it's wise to roll our own async utilities.
</aside>

## Logging

Since all logging in Core (and also in the SDK) is done via [tracing](https://docs.rs/tracing/latest/tracing/), users can just set up their own [subscriber](https://docs.rs/tracing/latest/tracing/trait.Subscriber.html) and get everything in whatever format they want. If they are using `log` already instead, there are [compatibility shims](https://docs.rs/tracing/latest/tracing/#log-compatibility) they can use for both directions.

## Metrics

Users will have a few options:

- Built-in OTel or Prometheus metrics support like langs have today
- Implement their own `CoreMeter` for arbitrary backing implementation
- Provide their own OTel [`PushMetricsExporter`](https://docs.rs/opentelemetry_sdk/0.31.0/opentelemetry_sdk/metrics/exporter/trait.PushMetricExporter.html) to use any OTel-compatible backend. This option needs to be added to the existing code, but should slot right in just fine.

# Big example (this all compiles)
